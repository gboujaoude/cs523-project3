...
public class ImmuneSystemModelApplication implements ApplicationEntryPoint{
    ...
    public void init() {
        ...
        Rectangle2D bloodstream = new Rectangle2D(streamMargin,0,
                Engine.getConsoleVariables().find(Singleton.SCR_WIDTH).getcvarAsFloat()-2*streamMargin,
                Engine.getConsoleVariables().find(Singleton.SCR_HEIGHT).getcvarAsFloat(),1000);
        bloodstream.addToWorld();

        PulseEntity simulation = new CellManager();
        Engine.getMessagePump().sendMessage(new Message(Singleton.ADD_PULSE_ENTITY, simulation));
